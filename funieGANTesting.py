'''
File to test the FunieGan model on test image data
'''
import os
import time
import ntpath
...
from utilityFiles.data_utils import getPaths, read_and_resize, preprocess, deprocess

## for testing arbitrary local data
data_dir = "data/test/testingImages/"
from utilityFiles.data_utils import get_local_test_data
test_paths = getPaths(data_dir)
print ("{0} test images are loaded".format(len(test_paths)))

## create dir for log and (sampled) validation data
samples_dir = "data/outputFinalImage/"
if not os.path.exists(samples_dir): os.makedirs(samples_dir)


checkpoint_dir  = 'Built_models/model_gen/'
model_name_by_epoch = "model_" 
